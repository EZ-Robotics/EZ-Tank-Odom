...
void Drive::drive_defaults_set() {
  ...
  pid_tuner_print_brain_set(true);
  pid_tuner_print_terminal_set(false);
  opcontrol_drive_reverse_set(false);
...
void Drive::pid_turn_encoder(bool input) {
  turn_with_encoder = input;
  drive_odom_enable(input);
}
void Drive::drive_imu_reset(double new_heading) {
  imu.set_rotation(new_heading);
  encoder_angle_rad = util::to_rad(new_heading);
  angle_rad = encoder_angle_rad;
  last_theta = encoder_angle_rad;
}
double Drive::drive_imu_get() {
  return turn_with_encoder ? odom_current.theta : imu.get_rotation();
}
